...
private let rcLinear3 = rcLine(base: 1, tree: rcNode(1, [rcNode(2, [rcNode(3)])]))   // 커먼 3형태: 6.25M/12.5M/18.75M
private let rcNoEvo = rcLine(base: 20, tree: rcNode(20))                              // 커먼 1형태: 37.5M 단일
...
final class RareCandyStoreTests: XCTestCase {
    ...
    /// 사탕 XP(5M) < 최소 임계(6.25M) → 진화 못 시키는 케이스는 부분 진행(.progressed), 통계 불변.
    ...
    func testUseEvolvesWhenCrossingThreshold() async {
        let s = store(rcLinear3)
        await s.hatch(baseID: 1)
        s.applyUsage(2_500_000)   // stage0(6.25M) 잔여 3.75M ≤ 5M
        giveCandies(s, 1)
        let result = s.useRareCandy()
    ...
    func testSingleCandyAdvancesAtMostOneStage() async {
        let s = store(rcLinear3)
        await s.hatch(baseID: 1)
        s.applyUsage(6_249_000)   // stage0 임계 직전
        giveCandies(s, 1)
        _ = s.useRareCandy()        // +5M → 11.249M: stage0(6.25M) 1회만, stage1(12.5M) 미달
    ...
    func testUseGraduatesFinalStage() async {
        ...
        s.applyUsage(33_000_000)   // 졸업 총량 37.5M 잔여 4.5M ≤ 5M
        giveCandies(s, 1)
        let result = s.useRareCandy()
    ...
    func testSequentialCandyUseMatchesDemo() async {
        let s = store(rcLinear3)
        await s.hatch(baseID: 1)
        s.applyUsage(RareCandy.xp)                      // stage0(6.25M) 도달 전
        giveCandies(s, 3)
        XCTAssertEqual(s.useRareCandy(), .evolved)     // 10M ≥6.25M → stage1, 이월 3.75M
        XCTAssertEqual(s.state.active?.stageIndex, 1)
        XCTAssertEqual(s.useRareCandy(), .progressed)  // 8.75M <12.5M → 부분성장
        XCTAssertEqual(s.state.active?.stageIndex, 1)
        XCTAssertEqual(s.useRareCandy(), .evolved)     // 13.75M ≥12.5M → stage2
