...
final class DittoRevealTests: XCTestCase {
    ...
    func testRevealAtFirstEvolution() async {
        ...
        s.applyUsage(accrued)   // 라인 미로딩 중 적립(첫 진화 임계 초과) — 진화/리빌 보류
        XCTAssertEqual(s.state.active?.usedAtStage, accrued)
        ...
        XCTAssertEqual(s.state.active?.usedAtStage, accrued - firstEvo, "첫 진화 초과분 이월")
    ...
    func testRevealRebasesDisguiseProfileAndLoadsDittoDetails() async {
        ...
        s.applyUsage(EconomyScale.tokens(300_000_000))
        await drainReveal(s)
        for _ in 0..<200 {
        ...
        XCTAssertEqual(profile?.growthTokens, EconomyScale.tokens(500_000_000))
    ...
    func testBoostedEasyDittoRetainsLevelAndIdentityAcrossReveal() async throws {
        ...
        s.applyUsage(EconomyScale.tokens(7_250_000)) // 6.25M reveal threshold plus 1M raw carryover.
        ...
        XCTAssertEqual(revealed.usedAtStage, EconomyScale.tokens(1_000_000))
    ...
    func testBoostedDisguiseRevealsAtTheHalvedThresholdAndKeepsTheBoost() async throws {
        ...
        s.applyUsage(EconomyScale.tokens(62_500_000))
        ...
        XCTAssertEqual(revealed.phaseThreshold, EconomyScale.tokens(1_500_000_000))
    }
    ...
    func testRevealClearsRepresentativePinnedToDisguiseAndFollowsDitto() async {
        ...
        s.applyUsage(PokemonBalance.phaseThreshold(rarity: .common, totalForms: 3, stageIndex: 0) + 1_000_000)
        await drainReveal(s)
    ...
    func testNoRevealBelowThreshold() async {
        let s = seedDisguise()
        s.applyUsage(1_000_000)   // 첫 진화 임계 미달
