...
**Detached.** Only the toolbar detach button undocks. Then it is a movable, resizable window with `fullSizeContentView`, a hidden transparent titlebar, and traffic lights on the sidebar shell. Root tabs and nested Linear/Collection filters live in a **resizable left sidebar**; **detach / Today / Settings** sit at the **bottom of that sidebar** (icon-only when the strip is collapsed). Collapse control is under the traffic lights. Frame autosave `PokeTokenBarMenuBarPanel`. No 500pt cap — min width grows with the sidebar (content **360** + sidebar + splitter + shell gaps), min height **400**, max **2400**. The same sidebar detach button snaps it back under the status item and locks it again.
...
**Detached.** Sidebar (Linear-like grey) holds those root tabs plus nested Linear/Collection filters; labels collapse to icons when they would wrap. Collapse button sits under the traffic lights; drag the splitter to resize (UserDefaults `menuBarSidebarWidth` / `menuBarSidebarCollapsed`, default **176pt**, min **148**, max **260**). The content column uses the same **inset rounded canvas** as attached (`popoverInsetCanvas`: white/dark page fill + hairline, 8pt shell gap on every side, below the titlebar) so the window is a grey app shell with a floating content panel. **detach/attach**, **Today**, and **Settings** sit on the shell at the bottom of the sidebar.
