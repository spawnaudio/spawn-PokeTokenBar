...
**Center** — issue + clock as the star (no companion HQ). ACTIVE ISSUE id + title (or Pomodoro title with no ID), huge remaining/OT clock, Pause / Open issue / Mark done / status, timer controls, notes, then 0:00 / check-in / forfeit / reset prompts. Empty: Focus idle copy, **Pomo Timer**, and “pin from the left list.”
